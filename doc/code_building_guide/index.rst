...
`Einstein Toolkit <https://einsteintoolkit.org/>`__ .  These fix upstream compatibility
...
From inside a TwoPunctures source tree, apply with either:

.. code-block:: bash

    # git checkout (preserves authorship + message)
    git am < /path/to/grace-src/extern/patches/twopunctures.patch

    # plain tarball (no git)
    patch -p1 < /path/to/grace-src/extern/patches/twopunctures.patch
...
.. list-table::
   ...
   * - GRACE_RECON_THERMO
     ...
     - Thermodynamic primitive reconstructed at faces. ``TEMP`` reconstructs temperature. ``PRESS`` reconstructs pressure.
   * - GRACE_Z4C_DER_ORDER
     - ``2`` | ``4`` | ``6`` (default ``6``)
