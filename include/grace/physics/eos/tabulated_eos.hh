    ...
    : public eos_base_t<tabulated_eos_t>
{
    ...
    press__eps_rho_ye_ymu_impl(double& eps, double& rho, double& ye, double& ymu, err_t& err) const
    {
        limit_rho(rho,err) ;
        limit_ye(ye,err)   ;
     ...
     */
    double GRACE_HOST_DEVICE
    press_temp__eps_rho_ye_ymu_impl(double& temp, double& eps, double& rho, double& ye, double& ymu, err_t& err) const
    {
        limit_rho(rho,err) ;
        limit_ye(ye,err)   ;
        auto lrho  = Kokkos::log(rho) ;
     ...
     */
    double GRACE_HOST_DEVICE
    press__temp_rho_ye_ymu_impl(double& temp, double& rho, double& ye, double& ymu, err_t& err) const
    {
        limit_rho(rho,err)   ;
        limit_ye(ye,err)     ;
        limit_temp(temp,err) ;
     ...
     */
    double GRACE_HOST_DEVICE
    eps__temp_rho_ye_ymu_impl(double& temp, double& rho, double& ye, double& ymu, err_t& err) const
    {
        limit_rho(rho,err)   ;
        limit_ye(ye,err)     ;
        limit_temp(temp,err) ;
    ...
    eps_h_csnd2_temp_entropy__press_rho_ye_ymu_impl( double& h, double& csnd2, double& temp
                                               ...
                                               , double& ye, double& ymu, err_t& err) const
    {
        Kokkos::abort("The function eps_h_csnd2_temp_entropy__press_rho_ye_ymu_impl is not well defined for a tabulated eos.") ;
    }
    /**************************************************************************************/
    /**************************************************************************************/
    /**************************************************************************************/
    ...
    eps_range__rho_ye_ymu_impl(double& eps_min, double& eps_max, double& rho, double& ye, double& ymu, err_t& err) const {
        limit_rho(rho,err) ;
        limit_ye(ye,err) ;
        double lrho = Kokkos::log(rho) ;
     ...
     */
    void KOKKOS_FUNCTION
    entropy_range__rho_ye_ymu_impl(double& s_min, double& s_max, double& rho, double& ye, double& ymu, err_t& err) const {
        limit_rho(rho,err) ;
        limit_ye(ye,err) ;
        double lrho = Kokkos::log(rho) ;
    ...
    limit_entropy_rho_ye(double& entropy, double& rho, double& ye, err_t& err) const
    {
        ...
        double smin, smax;
        double ymu_dummy{0} ;
        entropy_range__rho_ye_ymu(smin,smax,rho,ye,ymu_dummy,err) ;
        if ( entropy<smin) {
            entropy = smin ;
