...
namespace grace {
  ...
  struct froot_t {
    ...
    KOKKOS_FUNCTION froot_t(
      eos_t _eos, double _d, double _q, double _rsqr, double _rbsqr, double _bsqr, double _ye, double _ymu, double h0
    ) : eos(_eos), d(_d), qtot(_q), ye(_ye), ymu(_ymu), rsqr(_rsqr), bsqr(_bsqr), rbsqr(_rbsqr), brosqr(_rsqr*_bsqr-_rbsqr)
    {
    ...
    /**
     * @brief Get allowed eps range at rho, ye, ymu
     */
    void KOKKOS_INLINE_FUNCTION
    get_eps_range(double& epsmin, double& epsmax, double rho) const {
      ...
      eos_err_t eos_err ;
      double rhol{rho} ;
      eos.eps_range__rho_ye_ymu(epsmin,epsmax,rhol,yel,ymul,eos_err);
    }
    ...
    )
    {
      ...
      press = eos.press_h_csnd2_temp_entropy__eps_rho_ye_ymu(
        hh,csnd2,temp,entropy,eps,rho,ye,ymu,eos_err
      ) ;
    ...
    operator() (double mu)
    {
      ...
      // rho and eps are always in bound here
      eos_err_t eos_err ;
      double const press = eos.press__eps_rho_ye_ymu(
        eps,rho,ye,ymu,eos_err
      ) ;
    ...
    /************************************************/
    double d, qtot, ye, ymu ; //!< Density, energy density, ye, ymu
    /************************************************/
